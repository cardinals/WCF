...
{
    public class YAOXIEHQPSXX : IMessage<YAOXIEHQPSXX_IN, YAOXIEHQPSXX_OUT>
    {
        /// <summary>
        /// json字符串转换为Xml对象
        /// </summary>
        ...
        {
            OutObject = new YAOXIEHQPSXX_OUT();


            var ypptdz = System.Configuration.ConfigurationManager.AppSettings["YXPTDZ"];
            ypptdz = ypptdz + "/hospitalInterface/drug/distribute/getDistribute";
            ...
            //1、获取令牌
            System.Collections.Specialized.NameValueCollection postHosGetDistribute = new System.Collections.Specialized.NameValueCollection();
            postHosGetDistribute.Add("accessToken", InObject.JIEKOUDYPJ);
            postHosGetDistribute.Add("currentPageNumber", InObject.DANGQIANYM);
            postHosGetDistribute.Add("orderDetailInfo", inJason);



            try
            {
                byte[] byRemoteInfo = webClientObj.UploadValues(ypptdz, "POST", postHosGetDistribute);
                string strGetDistribute = System.Text.Encoding.UTF8.GetString(byRemoteInfo);

                strGetDistribute = strGetDistribute.Replace(":null", ":0");
                strGetDistribute = strGetDistribute.Replace("&lt;", "");
                strGetDistribute = strGetDistribute.Replace("&gt;", "");
                strGetDistribute = strGetDistribute.Replace("&;", "");

                var xmldom = new XmlDocument();
                var Xml = Json2Xml(strGetDistribute);
