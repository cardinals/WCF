...
{
    public class YAOXIETJZFDDMX : IMessage<YAOXIETJZFDDMX_IN, YAOXIETJZFDDMX_OUT>
    {
        /// <summary>
        /// json字符串转换为Xml对象
        /// </summary>
        ...
        {
            OutObject = new YAOXIETJZFDDMX_OUT();




            var ypptdz = System.Configuration.ConfigurationManager.AppSettings["YXPTDZ"];
            ypptdz = ypptdz + "/hospitalInterface/drug/payOrder/addOrderDetail";

            string inJason = "{\"list\":[";

            //采购订单明细
            var i = 0;
            foreach (var item in InObject.ZHIFUDDMX)
            {
                inJason = inJason + "{" +
                                "\"orderId\":\"" + item.DINDANBH + "\"," + 
                                "\"distributeId\":\"" + item.PEISONGMXBH + "\"," +  
                                "\"returnId\":\"" + item.TUIHUOMXBH+ "\"," +  
                                "\"orderCustomInfo\":\"" + item.ZIDINGYDDXX +"\"" +  
                                 "}";
